...
{
    seq_printf(p, " PWM_1, Period = 0x%08X, High time = 0x%08X\n", pwm_get_period(), pwm_get_high_time());
    return SUCCESS;
}
...
static const struct of_device_id zynq_pwm_1_match[] =
{
    {.compatible = "yarib-pwm_1-1.00.a"},
    { },
};

MODULE_DEVICE_TABLE(of, zynq_pwm_1_match);

/* platform driver structure for device driver */
static struct platform_driver zynq_pwm_1_driver =
{
    .driver =
    {
        .name = DRIVER_NAME,
        .owner = THIS_MODULE,
        .of_match_table = zynq_pwm_1_match
    },
    .probe = driver_probe,
...
module_platform_driver(zynq_pwm_1_driver);
